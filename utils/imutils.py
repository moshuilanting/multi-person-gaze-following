...
def multi_hot_targets_image(gaze_heatmap, threshold=0.2):
    target_gaze_heatmap = np.zeros(gaze_heatmap.shape)
    for i in range(len(gaze_heatmap)):
        for j in range(len(gaze_heatmap[0])):
            if gaze_heatmap[i][j]>threshold:
                target_gaze_heatmap[i][j]=1.0
    return target_gaze_heatmap
...
def gaussian_mixture(sigma,x,arr_u):
